...
struct ISBNTest {
    ...
    @Test func testRemoveSpaces() {
        ...
        let expected: UInt64 = 9780000000000 + registrant * 1000000 +
                publication * 10 + possibleCheck
        let actual: UInt64 = ISBN.removeSpaces(s)
        #expect(actual == expected)
    }
    
    @Test func testRemoveSeparatorsWhenThereAreNoneToRemove() {
        let expected: UInt64 = 9780000000000 +
                UInt64.random(in: 0 ... 10000000000)
        let numStr: String = expected.description
        let actual: UInt64 = ISBN.removeSeparators(numStr)
        #expect(actual == expected)
    }
    
    @Test func testRemoveSeparators() {
        let registrant = UInt64.random(in: 0 ... 999)
        let regStr = String(format: "%03d", registrant)
        let publication = UInt64.random(in: 0 ... 99999)
        let pubStr = String(format: "%05d", publication)
        ...
        let expected: UInt64 = 9780000000000 + registrant * 1000000 +
                publication * 10 + possibleCheck
        let actual: UInt64 = ISBN.removeSeparators(s)
        #expect(actual == expected)
    }
    ...
    /// Test that the constructor that takes a `String` converts a 10-digit
    /// number to ISBN-13, setting the correct ISBN-13 check digit. We will not
    /// worry whether the ISBN-10 number has a valid check digit or not.
    ...
    /// number to ISBN-13, setting the correct ISBN-13 check digit. We will not
    /// worry whether the ISBN-10 number has a valid check digit or not.
    @Test func testStringConstructorISBN10WithXToISBN13SetsCheckDigit() {
        let registrant = UInt64.random(in: 0 ... 999)
        let regStr = String(format: "%03d", registrant)
        let publication = UInt64.random(in: 0 ... 99999)
        let pubStr = String(format: "%05d", publication)
